...
<html lang="en-US"> 
  ...
  (function() {
    ...
    <body>
        ...
        <div id="work" class="page">
            <div class="container">
                <h2 style="text-align:center;">Proposi el 7all w mayfi9 bik 7ad </h2>
                <BR>
 ...
 if($_SERVER["REQUEST_METHOD"] == "POST") {
   ...
   $sql9="INSERT INTO `hal` (`id`, `hal`, `date`) VALUES ('$id', '$message', now())";
    $reponse9 = $bdd->query($sql9);
     echo "<h2> messagek teb3ath </h2>";
     echo "<BR>" ;
 }
...
$sql = "SELECT * FROM `hal`  WHERE id = '".$id."'ORDER BY likes DESC"; 
...
<form action="" method="post" class="cform-form">
                <center>    <textarea name="message" class="cform-textarea" cols="20" rows="13" style="width: 90%;" title="ekteb el 7all lel mochkel hedha "></textarea></center>
 <div  >
                 
                 <input  type="submit" value="Ab3eth " class="btn btn-primary btn-lg"  style="display: block; margin: 0 auto; width: 180px ">
 <br>               
 </div>
 </form>
...
while($data = $reponse2->fetch()) 
    ...
    $one ="'";
$dbl='"';
 print '<a href="choflihal2.php?id='.$dat.$dbl.' class="list-group-item active">';
 ...
 print '</font>';
  ...
  echo ' el hal: ' .$data['hal']; 
 print'</h4>' ;
  print '<p  class="list-group-item-text">';
  echo '   wa9tech : ' .$data['date'];
  echo "<BR>";
  ...
  print'<button type="button" onclick=showUser3('.$data['num'].',this) style="float: left;margin-right: 15px;width:150px" class="btn btn-success">Voti Pour&nbsp';
  print $data['likes']."</button>";

 print '<button type="button" onclick=showUser4('.$data['num'].',this) style="float: left;margin-right: 15px; width:150px" class="btn btn-danger">Voti Contre&nbsp';
...
xmlhttp.open("GET","getlikes2.php?q="+str,true);
xmlhttp.send();
}
function showUser4(str,bu)
{
if (str=="")
  {
...
xmlhttp.open("GET","getdislikes2.php?q="+str,true);
xmlhttp.send();
}
</script>
</html>